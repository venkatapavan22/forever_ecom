...
const Login = () => {
  ...
  const onSubmiHandler=async(e)=>{
    ...
    try {
      if(currentState==="Sign Up"){
        const response=await axios.post("http://localhost:4500/api/user/register",{name,email,password})
        ...
        const response=await axios.post("http://localhost:4500/api/user/login",{email,password})
        if(response.data.success){
          setToken(response.data.token)
          localStorage.setItem("token",response.data.token)
        }else{
  ...
  return (
    <form onSubmit={onSubmiHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800 '>
